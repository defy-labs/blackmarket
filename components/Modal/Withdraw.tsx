    ...
    const withdrawToChain = handleSubmit(async (formData) => {
        if (account && formData.transferAmount > 0) {
            ...
            try {
                ...
                const tx = await withdrawTokens(account, message, signedMessageHash || '')
                ...
                <ModalFooter>
                    ...
                    </Button>
                    {
                        ...
                        withdrawnState && (
                            <>
                                <Text as="div">
                                    {t('withdraw.withdrawnSuccessfully')}
                                </Text>
                                <Button
                                    size="lg"
                                    width="100%"
                                    onClick={e => window.open(`https://polygonscan.com/tx/${transactionHash}`, '_blank')}>
                                    {"Polygonscan"}
                                </Button>
                            </>
                        )
